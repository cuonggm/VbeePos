...
(function($) {
		...
		function Autocomplete(element, options) {
			...
			this.$container
					.append('<span class="input-group-btn">'
							...
							+ '</button></span>');
			this.$container
					.append('<ul class="items dropdown-menu" style="display:none;width:500px"></ul>');
			...
			this._validateOnHandler = function() {
				if (_this.options.validation) {
					if (!_this.options.validation(_this.$input.val(),
							_this.data)
							&& _this.options.invalidClass) {
			...
			$('body').click(
					function() {
						...
						if (!$active.is(_this3.$input)
								&& !$active.is(_this3.$button)
								&& !$active.is(_this3.$items)
								&& !_this3.$items.find($active).length) {
							_this3.open = false;
						}
					});
				...
				[
						...
						},
						{
							...
							value : function parseOptions(options) {
								...
								if (this.$input.data('value-field')) {
									this.$valueField = $(this.$input
											.data('value-field'));
								} else if (typeof this.options.valueField === "string") {
									this.$valueField = $(this.options.valueField);
								} else if (this.options.valueField
										&& this.options.valueField.length) {
									this.$valueField = this.options.valueField;
								}

								if (this.$input.data('append-to-body')) {
									this.options.appendToBody = !!this.$input
											.data('append-to-body');
								}
								...
								var preAppendDataItem = this.$input
										.attr("pre-append");
								if (preAppendDataItem) {
									this.options.preAppendDataItem = new Function(
											"li", "item", preAppendDataItem);
								}

								if (this.options.preAppendDataItem) {
									this.options.preAppendDataItem = this.options.preAppendDataItem
											.bind(this);
								}

								var validation = this.$input.attr("validation");
								if (validation) {
									this.options.validation = function(input,
											data) {
										return eval(validation);
									};
								}

								var invalidClassAttr = this.$input
										.data("invalid-class");
								if (invalidClassAttr) {
						...
						},
						{
							...
							value : function initializeEventHandlers() {
								if (this.options.openOnInput) {
									this.$input[0].addEventListener('input',
											this._openOnInputHandler);
								}
								this.$input[0].addEventListener('keydown',
										this._onKeyDown);
								this.$input[0].addEventListener('blur',
										this._onBlurHandler);
								this.$input[0].addEventListener(
										this.options.filterOn,
										this._filterOnHandler);
								this.$input[0].addEventListener(
										this.options.validateOn,
										this._validateOnHandler);
						...
						},
						{
							...
							value : function removeEventHandlers() {
								if (this.options.openOnInput) {
									this.$input[0].removeEventListener('input',
											this._openOnInputHandler);
								}
								this.$input[0].removeEventListener('keydown',
										this._onKeyDown);
								...
								this.$input[0].removeEventListener(
										this.options.validateOn,
										this._validateOnHandler);
								this.$button[0].removeEventListener('click',
										this._buttonClickHandler);
						...
						},
						{
							...
							value : function setButtonIcon() {
								this.$button.html('<span class="'
										+ (this.open ? 'fa fa-caret-up'
												: 'fa fa-caret-down')
										+ '"></span>');
							}
						},
						{
							...
							value : function search(input) {
								...
								var results = void 0;
								if (this.options.filter) {
									results = this.options.filter(input,
											this.data);
								} else {
									results = this.data;
								}
								...
								if (results && results.length && input) {
									if (this.options.distinct) {
										results = this.getUniqueValuesOfKey(
												results,
												this.options.nameProperty);
									}
									if (results.length === 1
											|| this.options.selectFirstMatch) {
										if (this.autoSelect
												|| this.options.selectFirstMatch) {
											this.selected = results[0];
											this.$input
													.val(results[0][this.options.nameProperty]);
						...
						},
						{
							...
							value : function buildDropdownItems(dataItems) {
								...
								if (this.options.distinct) {
									dataItems = this.getUniqueValuesOfKey(
											dataItems,
											this.options.nameProperty);
								}

								var liElements = dataItems
										.map(function(x) {
											var li = document
													.createElement('li');
											li
													.setAttribute(
															'value',
															x[_this.options.valueProperty]);
											li.innerHTML = '<a class="dropdown-item">'
													+ x[_this.options.nameProperty]
													+ '</a>';
											li
													.addEventListener(
															'click',
															function(e) {
																_this.$input
																		.val(x[_this.options.nameProperty]);
																_this.selected = x;
																_this.open = false;
															});
											if (_this4.options.preAppendDataItem) {
												_this4.options
														.preAppendDataItem(li,
																x);
											}
											return li;
										});
								...
								for (var i = 0, s = 400; i < liElements.length; i += s) {
									liElements.slice(i, i + s).forEach(
											function(li) {
						...
						},
						{
							...
							value : function loadDataSource() {
								...
								if (typeof this.datasource === "string") {
									// URL
									var _this = this;
									requestBundler.get(this.datasource,
											function(data) {
												_this.data = data;
						...
						},
						{
							...
							value : function selectInitialValue() {
								if (this.$valueField) {
									...
									this.$input
											.val(this.selected[this.options.nameProperty]);
								}
								this.$input
										.trigger(this.options.initialValueSelectedEvent);
							}
						},
						{
							...
							value : function setMenuDirection() {
								...
								requestAnimationFrame(function() {
									...
									var inputHeight = _this5.$input
											.outerHeight();
									var inputMarginTop = parseInt(_this5.$input
											.css('margin-top'));

									// let menuOffset = this.$items.offset();
									var menuHeight = _this5.$items
											.outerHeight();

									var vpHeight = $(window).height();

									var noSpaceBelow = inputOffset.top
											+ inputHeight + menuHeight > vpHeight;
									...
									if (noSpaceBelow && spaceAbove) {
										_this5.$items.offset({
											top : inputOffset.top - menuHeight
													- inputMarginTop,
						...
						},
						{
							...
							set : function set(val) {
								// reflect the value of the open property as an
								// HTML attribute
								if (val) {
						...
						},
						{
							...
							set : function set(value) {
								...
								// reflect the value of the selected property as
								// an HTML attribute
								if (!value) {
									value = {};
								}
								...
								if ((typeof value === 'undefined' ? 'undefined'
										...
										&& this.data) {
									if (!isNaN(+value)) {
										value = +value; // if value is a string
								...
								this.$container[0]
										.setAttribute(
												'selected',
												value[this.options.valueProperty]
														|| '');
								...
								if (this.$valueField) {
									this.$valueField
											.val(value[this.options.valueProperty]
													|| '');
								}
	...
	var defaultOptions = {
		...
		filter : function filter(input, data) {
			...
			return data.filter(function(x) {
				return ~x[_this7.options.nameProperty].toLowerCase().indexOf(
						input.toLowerCase());
			});
		},
		filterOn : 'input',
	...
	$.fn.autocomplete = function(option) {
		this
				.each(function() {
					var $this = $(this), data = $this.data('autocomplete'), options = (typeof option === 'undefined' ? 'undefined'
